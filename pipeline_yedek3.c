...
/* --- sadece son komutun heredoc’unu, forktan ÖNCE topla --- */
static int	collect_last_heredoc(t_cmd *tail, t_ms *ms)
{
	...
	hfd = prepare_heredoc_fd(tail, ms);
...
/* --- launch’tan sonra parent’ta tail heredoc fd’yi kapat --- */
static void	close_tail_heredoc_parent(t_cmd *tail)
{
	if (tail && tail->heredoc_fd >= 0)
	{
		close(tail->heredoc_fd);
		tail->heredoc_fd = -1;
	}
}
...
int	run_pipeline(t_cmd *cmds, t_ms *ms)
{
	...
	ms->heredoc_index = 1;
	tail = last_cmd(cmds);
	if (collect_last_heredoc(tail, ms) < 0)
	...
	if (in_fd != STDIN_FILENO)
		close(in_fd);
	close_tail_heredoc_parent(tail);
